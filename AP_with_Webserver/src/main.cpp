...
String header_login = "<head><meta http-equiv='content-type' content='text/html; charset=UTF-8'><meta charset='utf-8'><title>Login</title></head>";
...
void handleBasics(){
  ...
  //}
  //server.send(200, "text/html", (head + main_header + "<body><div id='body'>" + nav_bar_basics + "<form action='/basics' method='POST'>" + basics + save_discard + "</form></div>" + "<a href='/login?DISCONNECT=YES'>disconnect</a>" + footer + "</body>" + style + "</html>"));
  server.send(200, "text/html", (head + main_header + new_basics + "<a href='/login?DISCONNECT=YES'>disconnect</a>" + footer + "</body>" + style + "</html>"));
}

void handleDMX(){
  //server.send(200, "text/html", (head + main_header + "<body><div id='body'>" + nav_bar_dmx + "<form action='/dmx' method='POST'>" + dmx + save_discard + "</form></div>" + "<a href='/login?DISCONNECT=YES'>disconnect</a>" + footer + "</body>" + style + "</html>"));
  server.send(200, "text/html", (head + main_header + new_dmx + "<a href='/login?DISCONNECT=YES'>disconnect</a>" + footer + "</body>" + style + "</html>"));
}

void handleActions(){
  //server.send(200, "text/html", (head + main_header + "<body><div id='body'>" + nav_bar_actions + "<form action='/actions' method='POST'>" + actions + save_discard + "</form></div>" + "<a href='/login?DISCONNECT=YES'>disconnect</a>" + footer + "</body>" + style + "</html>"));
